...
static void vc_code_decoded(boost::system::error_code ec, std::string provider, std::string vccode, std::function<void()> reportbadvc)
{
...
static void decode_verify_code(boost::asio::io_service & io_service, const boost::filesystem::path vcodeimgfile, decaptcha::deCAPTCHA & decaptcha)
{
	...
	decaptcha.async_decaptcha(
		buffer,
		std::bind(&vc_code_decoded, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4)
	);
}
