...
public:
		...
		m_handler(handler)
	{
		...
		// 让 XMPP/IRC 的聊友版面
		m_io_service.post(
			bind_handler(*this, boost::system::error_code(), std::string("deCAPTCHA"), std::string(), std::function<void()>())
		);
	}

	template<class Functor>
	void operator()(boost::system::error_code ec, std::string provider, std::string result, Functor reportbad = std::function<void()>() )
	{
...
{
	return detail::async_decaptcha_op<DecoderOp,typename std::remove_reference<Handler>::type>(
...
class deCAPTCHA{
	typedef std::function<void()>	reportbadfunc_t;
