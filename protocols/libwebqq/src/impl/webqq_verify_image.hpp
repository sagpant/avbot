...
public:
	get_verify_image_op(std::shared_ptr<WebQQ> webqq, std::string vcimgid, webqq::webqq_handler_string_t handler)
		...
		m_buffer(std::make_shared<boost::asio::streambuf>())
	{
		...
		std::string url = boost::str(
			boost::format(LWQQ_URL_VERIFY_IMG) % APPID % m_webqq->m_qqnum % vcimgid
		);
