...
{
public:
	poll_message_op(std::shared_ptr<qqimpl::WebQQ> webqq,  Handler handler)
	...
	void operator()(boost::system::error_code ec, std::size_t bytes_transfered)
	{
		...
		m_jstree = std::make_shared<pt::wptree>();
		...
		if (retcode)
		{
			return m_webqq->get_ioservice().post(
				std::bind<void>(m_handler, ec)
			);
		}

		m_webqq->get_ioservice().post(
			std::bind<void>(*this, boost::system::error_code())
		);
	}
				...
				else if( poll_type == "sys_g_msg" )
				{
					...
					if( result->second.get<std::wstring>( L"value.type" ) == L"group_join" )
					{
						...
						m_webqq->m_group_refresh_queue.push(
							boost::make_tuple(
								// callback
								std::bind(&WebQQ::cb_newbee_group_join, m_webqq, group, newuseruid),
								groupnumber
							)
						);
...
private:
	...
	//
	std::shared_ptr<pt::wptree> m_jstree;
	boost::property_tree::wptree::iterator m_iterator, m_iterator_end;
};

template<class Handler>
detail::poll_message_op<Handler> make_poll_message_op(std::shared_ptr<WebQQ> webqq, Handler handler)
{
...
template<class Handler>
void async_poll_message(std::shared_ptr<WebQQ> webqq, Handler handler)
{
