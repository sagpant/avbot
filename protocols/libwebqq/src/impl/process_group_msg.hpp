...
{
	process_group_message_op(std::shared_ptr<WebQQ> webqq, const boost::property_tree::wptree& jstree, Handler handler)
		: m_jstree(std::make_shared<boost::property_tree::wptree>(jstree))
		, m_webqq(webqq)
		, m_handler(handler)
	{
			...
			else
			{
				if (!m_webqq->m_buddy_mgr.get_buddy_by_uin(who))
				{
					...
					BOOST_ASIO_CORO_YIELD m_webqq->update_group_member(
						...
						std::bind<void>(*this, std::placeholders::_1, url)
					);
...
private:
	...
	std::shared_ptr<boost::property_tree::wptree> m_jstree;
...
make_process_group_message_op(std::shared_ptr<WebQQ> webqq,
...
template<class Handler>
void process_group_message(std::shared_ptr<WebQQ> webqq,
