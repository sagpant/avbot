...
{
public:
	upload_offline_file_op(const std::shared_ptr<WebQQ> & _webqq,
		...
		, m_handler(handler)
		, m_filename(filename)
		, m_file(std::make_shared<avhttp::default_storge>())
	{
		...
		m_uploader = std::make_shared<avhttp::file_upload>(boost::ref(m_webqq->get_ioservice()), true);
	...
	void operator()(boost::system::error_code ec, std::size_t bytes_transfered = 0)
	{
		BOOST_ASIO_CORO_REENTER(this)
		{
			...
			m_buffer = std::make_shared< boost::array<char, 1024> >();
			...
			// 读取 body
			m_result = std::make_shared<boost::asio::streambuf>();
...
private:
	void failed_invode_handler(const boost::system::error_code & ec)
	{
		m_webqq->get_ioservice().post(
			std::bind<void>(m_handler, ec, std::string(), std::string())
		);
	}

private:
	std::shared_ptr<WebQQ> m_webqq;
	std::shared_ptr<avhttp::file_upload> m_uploader;
	std::shared_ptr<avhttp::default_storge> m_file;
	...
	std::shared_ptr<boost::asio::streambuf> m_result;
...
make_upload_offline_file_op(const std::shared_ptr<WebQQ> & _webqq, const std::string & filename,
...
template<class Handler>
void async_upload_offline_file(const std::shared_ptr<WebQQ> & _webqq, const std::string & filename,
	const std::string & from_uin, const std::string & to_uin, Handler handler)
{
...
{
public:
	send_offline_file_op(const std::shared_ptr<WebQQ>& _webqq,
...
make_send_offline_file_op(const std::shared_ptr<WebQQ> & _webqq,
...
template<class Handler>
void async_send_offline_file(const std::shared_ptr<WebQQ> & _webqq,
		const std::string &to_uin, const std::string &filename, Handler handler)
{
