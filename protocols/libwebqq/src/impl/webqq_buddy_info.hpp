...
class update_buddy_list_op : boost::asio::coroutine
{
	...
	void do_fetch()
	{
		...
		m_webqq->logger.dbg() << "getting buddy list";
...
public:
	update_buddy_list_op(const std::shared_ptr<WebQQ>& webqq, Handler handler)
		: m_webqq(webqq)
		, m_handler(handler)
	{
		do_fetch();
...
make_update_buddy_list_op(const std::shared_ptr<WebQQ> & _webqq, Handler handler)
{
...
template<class Handler>
void async_update_buddy_list(const std::shared_ptr<WebQQ>& _webqq, Handler handler)
{
