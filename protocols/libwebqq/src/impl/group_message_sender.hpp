...
{
public:
	group_message_sender_op(std::shared_ptr<qqimpl::WebQQ> webqq)
		: m_webqq(webqq)
	{
		// 进入循环吧.
...
private:
	void send_group_message(boost::tuple<std::string, std::string, WebQQ::send_group_message_cb> v)
	{
		...
		avhttp::async_read_body(
			...
			*m_buffer,
			std::bind<void>(*this, std::placeholders::_1, std::placeholders::_2, v)
		);
	}
private:
...
detail::group_message_sender_op group_message_sender(std::shared_ptr<Webqq> webqq)
{
