...
public:
	update_group_qqnumber_op(std::shared_ptr<WebQQ> webqq,
		...
		*/
		...
		try {
			...
			if( jsonobj.get<int>( "retcode" ) == 0 ) {
				...
				m_webqq->logger.dbg() <<  "qq number of group "
...
private:
	...
	std::shared_ptr<qqGroup> m_this_group;
	Handler m_handler;
...
make_update_group_qqnumber_op(std::shared_ptr<WebQQ> webqq, std::shared_ptr<qqGroup> group, Handler handler)
{
...
template<class Handler>
void async_update_group_qqnumber(std::shared_ptr<WebQQ> webqq, std::shared_ptr<qqGroup> group, Handler handler)
{
