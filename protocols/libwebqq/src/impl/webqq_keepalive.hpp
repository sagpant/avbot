...
public:
	webqq_keepalive_op(const std::shared_ptr<WebQQ>& webqq)
		: m_webqq(webqq)
	{
		start_poll();
	}
	...
	void operator()(const boost::system::error_code& ec, std::size_t bytes_transfered)
	{
		...
		{for (;m_webqq->m_status!= LWQQ_STATUS_QUITTING;){
			...
			BOOST_ASIO_CORO_YIELD boost::delayedcallsec(
				...
				std::bind<void>(*this, ec, bytes_transfered)
			);
...
webqq_keepalive_op make_keepalive_op(const std::shared_ptr<WebQQ>& webqq)
{
...
void webqq_keepalive(const std::shared_ptr<WebQQ>& webqq)
{
