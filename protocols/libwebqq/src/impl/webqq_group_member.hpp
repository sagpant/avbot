...
{
public:
	update_group_member_op(std::shared_ptr<WebQQ> webqq,
		std::shared_ptr<qqGroup> group, Handler handler)
		: m_webqq(webqq)
		, m_handler(handler)
		, m_buffer(std::make_shared<boost::asio::streambuf>())
	{
		...
		avhttp::async_read_body(
			*m_stream, url, * m_buffer,
			std::bind<void>( *this, std::placeholders::_1, std::placeholders::_2, group)
		);
	}

	void operator()(boost::system::error_code ec, std::size_t bytes_transfered, std::shared_ptr<qqGroup> group)
	{
		pt::ptree jsonobj;
		...
		{
			m_webqq->logger.err() <<  __FILE__ << " : " << __LINE__ << " : "
...
private:
	void group_member_process_json(pt::ptree& jsonobj, std::shared_ptr<qqGroup> group)
	{
		//TODO, group members
...
make_update_group_member_op(std::shared_ptr<WebQQ> webqq, std::shared_ptr<qqGroup> group, Handler handler)
{
...
template<class Handler>
void update_group_member(std::shared_ptr<WebQQ> webqq, std::shared_ptr<qqGroup> group, Handler handler)
{
