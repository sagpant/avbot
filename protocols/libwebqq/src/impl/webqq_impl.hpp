...
typedef std::shared_ptr<avhttp::http_stream> read_streamptr;
...
typedef std::map<std::string, std::shared_ptr<qqGroup> > grouplist;

typedef boost::system::error_code system_error;

class SYMBOL_HIDDEN WebQQ  : public std::enable_shared_from_this<WebQQ>
{
public:
	using std::enable_shared_from_this<WebQQ>::shared_from_this;
public:
	WebQQ(boost::asio::io_service & asioservice, boost::logger& _logger, std::string qqnum, std::string passwd, bool no_persistent_db=false);
	...
	void change_status(LWQQ_STATUS status, std::function<void (boost::system::error_code) > handler);
...
public:
	...
	void update_group_member_qq(std::shared_ptr<qqGroup> group );
	...
	void cb_search_group(std::string, const boost::system::error_code& ec, read_streamptr stream,  std::shared_ptr<boost::asio::streambuf> buf, webqq::search_group_handler handler);

	void fetch_aid(std::string arg, std::function<void(const boost::system::error_code&, std::string)> handler);
	void cb_fetch_aid(const boost::system::error_code& ec, read_streamptr stream,  std::shared_ptr<boost::asio::streambuf> buf, std::function<void(const boost::system::error_code&, std::string)> handler);

	void cb_join_group(qqGroup_ptr, const boost::system::error_code& ec, read_streamptr stream,  std::shared_ptr<boost::asio::streambuf> buf, webqq::join_group_handler handler);
